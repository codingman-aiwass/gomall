...
func (l *RefreshTokenByRPCLogic) RefreshTokenByRPC(in *auth.RefreshTokenReq) (*auth.RefreshTokenResp, error) {
	token, err := jwtx.RenewToken(l.svcCtx.Config.AuthConfig.RefreshSecret, l.svcCtx.Config.AuthConfig.AccessSecret, l.svcCtx.Config.AuthConfig.AccessExpire, in.AccessToken, in.RefreshToken)
	if err != nil {
		return nil, err
	}
